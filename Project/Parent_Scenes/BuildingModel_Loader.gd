...
func load_model() -> BuildingModel:
	...
	match declared_faction:
		factions.AMERULF:
			if !FileAccess.file_exists("res://Models/Amerulf/scene_files/"+get_parent().type.to_lower()+"_am.tscn"):
		...
		factions.TYRGWIS:
			if !FileAccess.file_exists("res://Models/Tyrgwis/scene_files/"+get_parent().type.to_lower()+"_ty.tscn"):
		...
		factions.CRIMELORD:
			if !FileAccess.file_exists("res://Models/CrimeLord/scene_files/"+get_parent().type.to_lower()+"_cl.tscn"):
